...
- **핵심 규칙 — 0x02 는 배열의 순수 0 마커다** (2026-07-07, 코퍼스 측정 확정):
  배열 원소로 나온 `0x02` 는 **payload 를 갖지 않고 값 0 을 뜻한다** (1바이트만
  소비). 진짜 값을 싣지 않으므로 지배 태그에도 영향을 주지 않는다.

  > **측정 근거** (`dd_analyze.py`, 605개): 모호성이 전혀 없는 **이름 있는
  > 스칼라 필드**에서 `0x02` 는 **602개 전부 값 0**(비영 0회)이었다. 반면 진짜
  > int8 값은 `0x04`(int8_c)가 싣는다(1087개, 비영 값 33종). 즉 `0x02` 는
  > "0/플래그" 전용 태그이며, 이 사실이 배열의 전부-`02` 축퇴
  > (`02 02…` = `[2,2,…]` vs `[0,0,…]`)를 **결정적으로 `[0,0,…]` 로** 확정한다.
  > 필드 이름에 의존하던 과거의 count 타이브레이크는 이 일반 규칙으로 대체·삭제됨.

- **그 외 스칼라(0x01/0x03/0x04/enum/bool/float)는 값을 싣는다.** 0 은 언제나
  `0x02` 로 기록되므로, 이들에는 배열 내 payload 생략(마커)이 없다 — 동질성상
  지배 태그와 같은 태그만 원소로 온다.
