...
- **1바이트 태그 전용 배열의 표현 축퇴와 count 필드 타이브레이크**
  (2026-07-07, 실측 logged_errors): 배열이 1바이트 태그로만 이어지는 구간은
  `02 02` = "int8 값 2 한 개" = "0 마커 두 개" 로 **바이트 표현 자체가
  축퇴**되어 문법으로는 원리적으로 해소 불가다. 이때 writer 가 배열 옆에
  기록하는 **count 성 형제 필드**(`nr_of_<이름>`, `nr_<이름>`, `num_<이름>`,
  `<이름>_count`)가 유일한 심판이 된다 — 두 해석의 배열 길이 중 count 와
  일치하는 쪽을 채택 (`dd_strict.py` 의 count-field 타이브레이크).
  count 필드가 없으면 ambiguous 로 정직하게 보고.
