...
- 판정법:
  - **1바이트 스칼라 태그**(`01/02/03/04`): payload 후보 바이트 자체가 유효한
    원소 시작(`0x00`~`0x0B`)이면 **0 생략 마커로 확정**. 연속 드롭아웃
    (`02 02` = 0 두 개)을 "int8 값 2"로 오독하는 모호성을 없애는 결정 규칙 —
    실측상 배열의 1바이트 태그는 0 표기로만 관측됨 (0이 아닌 작은 값은
    지배 타입 그대로, 예: enum `05 00 02` 로 기록되는 것으로 추정).
  - **2바이트 이상 스칼라**: payload 로 읽은 뒤 **직후 바이트가 유효한 원소
    시작이 아니면** 생략으로 재해석. (`0x19` 같은 "미지 태그" 실패는 대부분
    이 생략을 몰라 payload 를 삼킨 디싱크의 잔해다.)
- 0값 원소가 배열의 지배 타입(enum 등)과 다른 태그(`0x02` int8)로 기록되는
  것도 관측됨 — writer 가 0을 가장 짧은 표현으로 쓰는 것으로 추정.
