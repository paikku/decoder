...
- **핵심 불변식 — 배열 동질성** (2026-07-07 정련, 실코퍼스 ambiguous 분석 반영):
  한 배열의 원소는 **같은 카테고리**(수치 / 문자열 / 컨테이너)를 가진다.
  0 은 아래 **0x02·0x03 = 0 마커** 로 기록되고, **수치 카테고리 안에서는
  값 크기에 따라 태그 폭이 바뀔 수 있다**(승격, 아래 (3)).

  ```
  05 23 3d │ 02 │ 05 19 19 │ 05 18 a8    ← 수치 배열, 02=0 마커
  └ 9021 ─┘ └0┘ └ 6425 ──┘ └ 6312 ──┘
  ```
...
- **그 외 스칼라(0x04/enum/bool/float)는 값을 싣는다.** 0 은 언제나 `0x02`/`0x03`
  마커로 기록되므로, 이들에는 배열 내 payload 생략이 없다.

**(3) 수치 배열은 값 크기에 따라 타입이 승격된다** (2026-07-07, 실측):
배열은 *엄격 동질*이 아니라 *카테고리 동질*이다 — 수치 원소는 값이 좁은
타입 범위를 넘으면 더 넓은 수치 타입으로 바뀐다.

```
ratios 실측 (등차 +5 수열):
   04 22 04 27 … 04 7c │ 05 00 81 05 00 86 …
   └ int8_c: 34,39,…,124 ┘ └ enum: 129,134,… ┘
   → [34,39,44,…,119,124, 129,134,…,174]
      └ int8 범위(≤127) ─┘ └ 128↑ 은 enum(2B)로 승격 ┘
```

- 판별: 원소는 각자의 **자기 태그**로 폭이 결정되므로 (0x04→1B, 0x05→2B,
  0x08→8B …) 승격이 섞여도 파싱은 결정적이다. 파서는 수치 태그
  (`0x04~0x08`)를 한 카테고리(`NUM`)로 묶어 폭이 다른 원소를 허용한다.
- 카테고리 교차(수치↔문자열↔컨테이너)는 실제 배열에 없으므로 디싱크
  신호로 취급한다.
